...
class User {
  ...
  static async newAdmin(userName,firstName, lastName,password) {
    ...
    try {
      ...
      const newAdmin= await db.users.create({firstName, lastName,userName,password: hashedPassword, isAdmin: true});
 ...
 static async newStaff(userName,firstName, lastName,password,admin) {
    try {
      ...
      let newStaff = await db.user.create({firstName, lastName,userName,password: hashedPassword, isAdmin: false})
 ...
 static async getUserById(userId) {
  ...
  //now updateuser
  static async updateUserById(userId, parameter,value) {
    try {
    ...
    let user = await db.user.update({[parameter]:value}, {
        where: { id: userId }  
    })
    console.log(user)
    if(user<=0)
